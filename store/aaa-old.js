import { firestoreAction } from 'vuexfire'
// import createPersistedState from 'vuex-persistedstate'
import firebase from '~/plugins/firebase'
import 'firebase/app'
// import 'firebase/firestore' // ここには使用するFirebaseSDKモジュールを記載
import 'firebase/firebase-firestore' // ここには使用するFirebaseSDKモジュールを記載

const db = firebase.firestore()
const aaaRef = db.collection('aaa')

export const state = () => ({
  aaa: [],
})

export const actions = {
  // nuxtClientInit({ commit, state, dispatch }, { req }) {
  //   createPersistedState()(this)
  // },
  // init: firestoreAction(({ bindFirestoreRef }) => {
  init: firestoreAction(({ bindFirestoreRef }, ref) => {
    // bindFirestoreRef('aaa', aaaRef)
    bindFirestoreRef('aaa', ref)
  }),
  // add: firestoreAction((context, page) => {
  add: firestoreAction((context, { pageNo, page }) => {
    // console.log('titleName in store/aaa.js: ' + page.titleName)
    aaaRef.doc(pageNo).collection(pageNo).add({
      page,
      created: firebase.firestore.FieldValue.serverTimestamp(),
    })
  }),
  // update: firestoreAction((context, page) => {
  update: firestoreAction((context, { pageNo, id, page }) => {
    // console.log('id in db_update: ' + id)
    // aaaRef.doc(id).update({
    aaaRef.doc(pageNo).collection(pageNo).doc(id).update({
      page,
    })
  }),
  // remove: firestoreAction((context, id) => {
  remove: firestoreAction((context, { pageNo, id }) => {
    // aaaRef.doc(id).delete()
    aaaRef.doc(pageNo).collection(pageNo).doc(id).delete()
  }),
  // getPageByPages: firestoreAction((context, pageNo) => {
  //   // console.log('pages ---------- in aaa.js: ' + pages)
  //   return aaaRef
  //     .doc(pageNo)
  //     .collection(pageNo)
  //     .get()
  // }),
  toggle: firestoreAction((context, dbPage) => {
    aaaRef.doc(dbPage.id).update({
      done: !dbPage.done,
    })
  }),
}

export const getters = {
  getMyPageById: (state) => (id) => {
    // console.log('id in getMyPageById: ' + id)
    // 以下は全検索
    // getMyPageById: (state) => {
    // return state.aaa
    return state.aaa.find((dbPage) => dbPage.id === id)
  },
  // 多分以下の orderdDbPages は使われていない
  orderdDbPages: (state) => {
    // eslint-disable-next-line no-undef
    return _.sortBy(state.aaa, 'created')
  },
  // ユーザが登録した日付で逆順ソート
  orderdMainDate: (state) => {
    // return _.sortBy(state.aaa, 'page.fileUrl')
    // eslint-disable-next-line no-undef
    // return _.sortBy(state.aaa, 'page.fileUrl').reverse()
    // eslint-disable-next-line no-undef
    const tempData = _.sortBy(state.aaa, 'page.fileUrl').reverse()
    // console.log('tempData ----- in aaa: ', tempData)
    return tempData
  },
}
