<template>
  <div class="container-fluid">
    <div v-for="dbHeader in dbHeaders" :key="dbHeader.id" class="m-0 p-0">
      <h5 class="text-light text-center m-0 p-1" :style="commonData.buttonColorBasicSet">{{ dbHeader.myData.m52 }}</h5>
    </div>
...
const filesRef = storageRef.child('ddd/d52')
...
export default {
  ...
  computed: {
    ...
    dbPages() {
      // return this.$store.getters['ddd/orderdMainDate']('d52')
      return this.$store.getters['ddd/orderdMainDate']

      // return this.$store.dispatch('ddd/getPageByPages', 'd52')
      // const tempData = this.$store.dispatch('ddd/getPageByPages', 'd52').then((result) => {
      // eslint-disable-next-line vue/no-async-in-computed-properties
      // this.$store
      //   .dispatch('ddd/getPageByPages', 'd52')
      //   .then((result) => {
