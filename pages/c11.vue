<template>
  <div class="container-fluid">
    <div v-for="dbHeader in dbHeaders" :key="dbHeader.id" class="m-0 p-0">
      <h5 class="text-light text-center m-0 p-1" :style="commonData.buttonColorBasicSet">{{ dbHeader.myData.m11 }}</h5>
    </div>
...
const imagesRef = storageRef.child('ccc/c11')
...
export default {
  ...
  computed: {
    ...
    dbPages() {
      // return this.$store.getters['ccc/orderdDbPages']('c11')
  ...
  methods: {
    test1() {
      // console.log('1-1 ----------- in c11.vue test1(): ')
      ...
      // this.$store.dispatch('ccc/updateCommon', this.cCommonWork.text1name)
