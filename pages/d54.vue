<template>
  <div class="container-fluid">
    <div v-for="dbHeader in dbHeaders" :key="dbHeader.id" class="m-0 p-0">
      <h5 class="text-light text-center m-0 p-1" :style="commonData.buttonColorBasicSet">{{ dbHeader.myData.m54 }}</h5>
    </div>
...
const filesRef = storageRef.child('ddd/d54')
...
export default {
  ...
  computed: {
    ...
    dbPages() {
      // return this.$store.getters['ddd/orderdMainDate']('d54')
      return this.$store.getters['ddd/orderdMainDate']

      // return this.$store.dispatch('ddd/getPageByPages', 'd54')
      // const tempData = this.$store.dispatch('ddd/getPageByPages', 'd54').then((result) => {
      // eslint-disable-next-line vue/no-async-in-computed-properties
      // this.$store
      //   .dispatch('ddd/getPageByPages', 'd54')
      //   .then((result) => {
